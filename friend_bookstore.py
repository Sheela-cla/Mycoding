'''So your task today is to create a console-based bookstore application.
In other words when a user starts a program user should be able to see a welcome message and a menu like the following:
 
"--- Hello and welcome to the awesome Lexicon bookstrore! ---"
Add a book
View all books
Search a book by titel
Update a book price
Delete a book
...
def search_book_by_title():
    complete= input("Do you know complete name or part of it if complete press 1 else 0: ")
    if complete==1:
       ...
       regex_pattern = pymongo.MongoClient.re.compile(substring, pymongo.MongoClient.re.IGNORECASE)

      # Perform the query to find books with titles containing the substring
       matching_books = books.find({"title": regex_pattern})

       # Print the titles of the books that match the query
       print("Books containing the substring:", substring)
...
def main():
    while True:
        ...
        if user_choice == "1":
            ...
            print("Thank you for using the Camellia bookstore system!")
            break
        else:
